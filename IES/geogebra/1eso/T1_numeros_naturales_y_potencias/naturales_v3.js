...
NaturalesGame.prototype.running = function () {
    return !this.paused && !this.confirmReset && !this.confirmFinish &&
...
NaturalesGame.prototype.leaders = function () {
    var result = [], best = -1;
    for (var i = 0; i < this.teams.length; i++) {
        var score = this.teams[i].score;
        if (score > best) { best = score; result = []; }
        if (score === best) result.push(i);
    }
    return result;
};
...
NaturalesGame.prototype.finish = function () {
    this.phase = "over";
    this.confirmFinish = false;
    this.eligible = [];
    var winners = this.leaders();
    this.message = (winners.length === 1 ? "Gana: " : "Ganan: ") +
        winners.map(function (index) { return "Grupo " + (index + 1); }).join(", ");
    this.tone = "success";
};
...
NaturalesGame.prototype.dispatch = function (action, value) {
    if (this.confirmFinish) {
        if (action === "cancelFinish") this.confirmFinish = false;
...
var NV3 = {
    ...
    position: function (name, x, y) {
        var key = "position:" + name, value = x + "," + y;
        if (this.cache[key] === value) return;
        if (this.api.evalCommand("SetCoords(" + name + ", " + x + ", " + y + ")")) {
            this.cache[key] = value;
        }
    },
    render: function () {
        var self = this, g = this.game, visible = {}, p = NATURALES_COLORS;
        function show(name) { visible[name] = true; }
        function text(name, value) { show(name); self.set("setTextValue", name, value); }
        function icon(name, available, enabled) { show(name + (!available ? "Used" : enabled ? "" : "Off")); }
        var hide = g.paused || g.confirmReset || g.confirmFinish;
        ...
        try {
            if (g.paused) {
            ...
            } else if (g.confirmFinish) {
                show("finishTitle"); show("finishQuestion");
                ...
                if (g.phase === "setup") {
                    ...
                    var leaders = g.leaders(), over = g.phase === "over";
                    var best = g.teams[leaders[0]].score;
                    if (over) {
                        show("winnerPoster"); show("winnerCup"); show("winnerThanks");
                        text("winnerTitle", leaders.length === 1 ? "Ganador" : "Ganadores");
                        text("winnerScore", best + (best === 1 ? " punto" : " puntos"));
                        this.color("winnerTitle", p.gold);
                        this.color("winnerScore", p.gold);
                        this.color("winnerThanks", p.teal);
                        if (leaders.length === 1) {
                            text("winnerSolo", "Grupo " + (leaders[0] + 1));
                            this.color("winnerSolo", p.ink);
                            show("winnerSoloBadge");
                        } else {
                            for (var slot = 0; slot < leaders.length; slot++) {
                                text("winnerName" + slot, "Grupo " + (leaders[slot] + 1));
                                this.color("winnerName" + slot, p.ink);
                                show("winnerBadge" + slot);
                            }
                        }
                        ...
                        for (var i = 0; i < 4; i++) {
                            ...
                            this.color("answer" + i, reveal && i === q.correct ? p.green : excluded ? p.red : p.ink);
                            this.background("answer" + i, reveal && i === q.correct ? p.greenSoft : excluded ? p.redSoft : p.option);
                            if (excluded) show("discard" + i);
                        }
                        ...
                        this.background("status", g.tone === "failure" ? p.failure : g.tone === "success" ? p.success : p.option);
                        this.color("status", g.tone === "failure" ? p.red : g.tone === "success" ? p.green : p.teal);
                        var remaining = Math.max(0, Math.ceil(g.limit - g.elapsed - 0.000001));
                        var urgent = g.running() && g.limit > 0 && remaining <= 10;
                        show(urgent ? "clockFaceUrgent" : "clockFace");
                        if (g.limit) {
                            var clockName = urgent ? "clockUrgent" : "clock";
                            var digits = String(remaining);
                            text(clockName, digits);
                            // Fixed circle; center the digits without moving surrounding controls.
                            this.position(clockName, Math.round(772 - digits.length * (urgent ? 15.5 : 9.5)), urgent ? 114 : 108);
                            show("clockUnit");
                            this.color("clockUnit", urgent ? p.red : p.teal);
                        } else {
                            show("clockUnlimited");
                            this.color("clockUnlimited", p.teal);
                        }
                        this.color("clockUrgent", p.red);
                        this.color("clock", p.teal);
                        if (g.phase === "active" || g.phase === "rescue") {
                            var team = g.teams[g.actor()];
                            var normal = g.phase === "active";
                            icon("helpPass", team.pass, normal);
                            icon("helpTime", team.time, normal && g.limit > 0);
                            icon("helpRemove", team.remove, normal);
                            text("extraAmount", team.time ? "+" + g.settings.extra + " s" : "Usado");
                            this.color("extraAmount", !team.time ? p.red : normal && g.limit > 0 ? p.teal : p.muted);
                        }
                    ...
                    text("turn", over ? "Partida finalizada" :
                        (g.phase === "rescue" ? "Revive: Grupo " : "Turno: Grupo ") + (g.actor() + 1));
                    this.color("turn", over ? p.gold : p.teal);
                    for (var k = 0; k < g.teams.length; k++) {
                        var t = g.teams[k];
                        var current = !over && g.actor() === k;
                        var leader = leaders.indexOf(k) !== -1 && (over || best > 0);
                        text("team" + k, "Grupo " + (k + 1) + "   " + t.score + (t.score === 1 ? " punto" : " puntos") + (t.alive ? "" : "   Descanso"));
                        this.color("team" + k, over && leader ? p.gold : t.alive ? p.ink : p.muted);
                        this.background("team" + k, over && leader ? p.goldSoft : current ? p.tealSoft : p.paper);
                        if (leader) show("teamCup" + k);
                        ["pass", "time", "remove", "revive"].forEach(function (kind) {
                            icon("team" + k + "_" + kind, t[kind], kind === "revive" ||
                                (t.alive && (kind !== "time" || g.settings.seconds > 0)));
                        });
...
function naturalesClock() { NV3.clock(); }
function ggbOnInit() { NV3.mount(ggbApplet); }
